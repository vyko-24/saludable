...
export default function UserAccList(props) {
  ...
  const getGroups = async (userid) => {
    if (!userid) return;
      ...
      try {
          ...
          console.log(error);
          if(error.response.status === 400){
            setMsg('No hay grupos registrados');
          }else{
            setMsg('Error. Inténtelo más tarde');
          }
          setMessage(true);
  ...
  const formatMoney = (amount) => {
    ...
    const hasCreateAccRole = rol.some(role => role.name === 'Crear Cuentas');
    const hasUpdateAccRole = rol.some(role => role.name === 'Editar Cuentas');
    const hasCreateGrRole = rol.some(role => role.name === 'Crear Grupos');
    const hasUpdateGrRole = rol.some(role => role.name === 'Editar Grupos');
                  ...
                  <>
                      ...
                      <Dialog.Actions>
                          <Dialog.Button title="Editar" onPress={() =>
                              navigation.navigate('EditGr', { selectedGroup, edit: true})} disabled={!hasUpdateGrRole}/>
                          <Dialog.Button title="Agregar cuenta" onPress={() => navigation.navigate('CreateAcc', { selectedGroup, gr: true,})} disabled={!hasCreateAccRole}/>
                      </Dialog.Actions>
                  </>
              )}
          </Dialog>
                                              ...
                                              <ListItem.Swipeable
                                                  ...
                                                  leftContent={(reset) => (
                                                      <Button
                                                          ...
                                                          onPress={() => {
                                                              reset();
                                                              navigation.navigate('InfoAcc', { account, edit: false,});
                                                          }}
                                                  ...
                                                  rightContent={(reset) => (
                                                      <Button
                                                          onPress={() => {
                                                              reset();
                                                              navigation.navigate('InfoAcc', { account, edit: true});
                                                          }}
                                                          icon={{ name: 'edit', color: 'white' }}
                                                          disabled={!hasUpdateAccRole}
                              ...
                              <ListItem.Swipeable
                                  ...
                                  leftContent={(reset) => (
                                      <Button
                                          ...
                                          onPress={() => {
                                              reset();
                                              navigation.navigate('InfoAcc', { account: l, edit: false,});
                                          }}
                                          icon={{ name: 'info', color: 'white' }}
                                          buttonStyle={{ minHeight: '100%' }}
                                      />
                                  ...
                                  rightContent={(reset) => (
                                      <Button
                                          onPress={() => {
                                              reset();
                                              navigation.navigate('InfoAcc', { account: l, edit: true,});
                                          }}
                                          icon={{ name: 'edit', color: 'white' }}
